...
def smoothAdd1(pc, data, tagset):
    ...
    """
    pwt = {(w, t): ((get_prob(pc[0], (w, t)) + 1) / (get_prob(pc[1], t) + len(wordset) * len(tagset))) for w in words
           for
           t in tagset}
    return pwt
...
tagged = viterbi([w for (w, _) in dataS], tagsetT, wordsetT, pwt,
                 pt)  # TODO: nespoustet na cely text, ale pouze na casti
