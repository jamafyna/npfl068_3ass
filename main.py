...
from classes import Pwt, LinearSmoothedDistribution
import numpy as np
import sys

from nltk import UnigramTagger

file_name = sys.argv[1]  # 'data/texten2.ptg'
...
# dataH = [('###', '###'), ('###', '###'), ('A', 'A'), ('B', 'B'), ('~~~', '~~~'), ('~~~', '~~~')]
# dataS = [('###', '###'), ('###', '###'), ('A', 'A'), ('C', 'C'), ('~~~', '~~~'), ('~~~', '~~~')]
p, trig_tag_set = get_initial_parameters([t for s in data_T for (_, t) in s])
...
for sentence in data_S:
    prediction = vite(sentence, tagsetT, pwt, ptt)
    # at the beginning there should be two ###
    # at the end there should be two ~~~
    if prediction[0] != '###' or prediction[1] != '###':
        print('ERROR: Something wen horribly wrong')
        continue
